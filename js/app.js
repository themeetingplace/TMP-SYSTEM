...
// helper 只能看: 總覽 / 房況 / 合約 (檢視) / 房租查帳 (催繳結帳) / 維修 / 租客
// 不能看: 物件管理 (#properties) / 總收支 (#finance) / 報表 / 設定 / 帳號管理
const HELPER_ALLOWED = new Set(['dashboard', 'occupancy', 'tenants', 'unsettled', 'contracts', 'maintenance']);
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (window.__currentRole === 'helper') {
        ...
        // 「帳務管理」(#finance) 改成「房租查帳」(#unsettled) — helper 只能看後者
        // (audit: #unsettled 原本是 finance 的 sub-tab, helper 沒 finance 入口會找不到)
